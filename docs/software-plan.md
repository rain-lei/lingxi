...
10. 可执行任务：行动型输入保存为服务器任务卡，并在演示页和控制台完成确认、完成、取消与恢复；支持日期、相对日期和本/下周星期的明确时间，图片提取的时间也可进入调度；提前时长只接受用户输入或已召回偏好，并显示到期状态。
...
| GET | `/api/tasks?device_id=...` | 获取当前匿名访客的任务队列，可按状态筛选 |
| GET | `/api/device/protocol` | 获取 0.4-draft 设备协议清单 |
| POST | `/api/interactions` | 发起交互并流式接收事件 |
| POST | `/api/tasks/update` | 更新一条属于当前匿名访客的任务状态 |
...
- `tasks`：任务类型、标题、摘要、时间/地点文本、清单、来源和状态。
...
- 每个访客最多保留最近 100 条任务；任务只允许所属匿名访客读取和更新。
...
- 行动型输入会产生可重新查询的任务 ID，跨页面更新状态后保持一致；显式日期、时间与提前时长会进入服务端调度，已召回的提醒偏好可作为后续相似任务的默认提前时长，明确输入或“不提醒”优先。
