# 灵犀挂件软件规划（Software-first MVP）

版本：0.1 Demo  
目标：先在无硬件条件下证明“可运行、可体验、可验证”的软件闭环，再用 ESP32-S3 替换浏览器设备模拟器。
...
- 完成阶跃 `stepaudio-2.5-asr`、`stepaudio-2.5-chat`、`stepaudio-2.5-tts` 服务端接入。
- 用浏览器模拟硬件，验证 5 分钟演示脚本。
