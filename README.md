...
- 可执行任务中心：行动型输入自动保存为任务卡，支持待确认、已确认、已完成和已取消状态
...
没有 API Key 时系统使用确定性的 Mock AI，确保黑客松现场仍可演示。当前已实现持久化任务卡及其确认、完成、取消闭环；定时触发提醒仍属于下一阶段，页面不会把尚未实现的后台调度包装为已完成。ESP32-S3 WebSocket、I2S 音频和 OTA 属于 Device 0.5 实机阶段；当前页面只将已验证链路标记为“已完成”。完整阶段指标见[持续升级路线](docs/upgrade-roadmap.md)。
